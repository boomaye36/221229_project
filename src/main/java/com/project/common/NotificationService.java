...
public class NotificationService {
    ...
    public NotificationService(EmitterRepository emitterRepository) {
        this.emitterRepository = (EmitterRepository) emitterRepository;
    }
	 ...
	 * emitterRepository.findAllStartWithById(id); sseEmitters.forEach( (key,
	 * emitter) -> { // 데이터 캐시 저장(유실된 데이터 처리하기 위함)
	 * emitterRepository.saveEventCache(key, notification); // 데이터 전송
	 * sendToClient(emitter, key, NotificationResponse.from(notification)); } ); }
	 * 
	 * private Notification createNotification(Member receiver, Review review,
	 * String content) { return Notification.builder() .receiver(receiver)
	 * .content(content) .review(review) .url("/reviews/" + review.getId())
	 * .isRead(false) .build(); }
	 */
